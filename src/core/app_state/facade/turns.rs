...
impl TiangongState {
    ...
    pub(in crate::core::app_state) fn apply_assistant_delta(&mut self, delta: &ModelStreamChunk) {
        let service = self.services.turn_service;
    ...
    pub(in crate::core::app_state) fn mark_pending_turn_executing(&mut self, plan: &TaskPlan) {
        let service = self.services.turn_service;
        service.mark_pending_turn_executing(self, plan)
    }

    pub(in crate::core::app_state) fn finish_pending_turn_success(&mut self, exec: TurnExecution) {
        let service = self.services.turn_service;
        service.finish_pending_turn_success(self, exec)
    }

    pub(in crate::core::app_state) fn finish_pending_turn_error(&mut self, err_msg: &str) {
        let service = self.services.turn_service;
