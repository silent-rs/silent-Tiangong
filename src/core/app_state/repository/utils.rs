...
pub(in crate::core::app_state) fn cleanup_empty_skill_install_dirs(
    removed_path: &Path,
    install_root: &Path,
) -> Result<()> {
...
pub(in crate::core::app_state) fn converted_stage_cleanup_dir(
    install_path: &Path,
    converted: bool,
) -> Option<PathBuf> {
    if !converted {
...
pub(in crate::core::app_state) fn default_storage_root() -> PathBuf {
    user_storage_root()
}

pub(in crate::core::app_state) fn default_app_storage_path() -> PathBuf {
    default_storage_root().join("app.json")
}

pub(in crate::core::app_state) fn default_skills_config_path() -> PathBuf {
    default_storage_root().join("skills.json")
}

pub(in crate::core::app_state) fn default_mcp_config_path() -> PathBuf {
    default_storage_root().join("mcp.json")
}

pub(in crate::core::app_state) fn default_mcp_capability_cache_path() -> PathBuf {
    default_storage_root().join("mcp-tools-cache.json")
}

pub(in crate::core::app_state) fn default_sessions_dir_path() -> PathBuf {
    default_storage_root().join("sessions")
}

pub(in crate::core::app_state) fn default_legacy_storage_path() -> PathBuf {
    default_storage_root().join("sessions.json")
}

pub(in crate::core::app_state) fn default_skills_storage_dir_path() -> PathBuf {
    default_storage_root().join("skills")
}

pub(in crate::core::app_state) fn default_skills_lock_path() -> PathBuf {
    default_skills_storage_dir_path().join("skills-lock.json")
}

pub(in crate::core::app_state) fn default_mcp_lock_path() -> PathBuf {
...
pub(in crate::core::app_state) fn user_home_dir() -> Option<PathBuf> {
...
pub(in crate::core::app_state) fn session_storage_path(
    sessions_dir: &Path,
    session_id: &str,
) -> PathBuf {
...
pub(in crate::core::app_state) fn ensure_parent_dir(path: &Path) -> Result<()> {
...
pub(in crate::core::app_state) fn ensure_dir(path: &Path) -> Result<()> {
    fs::create_dir_all(path).with_context(|| format!("创建目录失败：{}", path.display()))
}

pub(in crate::core::app_state) fn copy_dir_recursive(src: &Path, dst: &Path) -> Result<()> {
    if !src.is_dir() {
...
pub(in crate::core::app_state) fn canonical_scru128_id(raw: &str) -> Option<String> {
    raw.trim()
...
pub(in crate::core::app_state) fn new_scru128_string() -> String {
    scru128::new().to_string()
}

pub(in crate::core::app_state) fn elapsed_ms_u64(value: u128) -> u64 {
    value.min(u128::from(u64::MAX)) as u64
}

pub(in crate::core::app_state) fn normalize_model_list(
    models: Vec<String>,
    current_model: &str,
) -> Vec<String> {
...
pub(in crate::core::app_state) fn validate_agent_config(config: &AgentConfig) -> Result<()> {
...
pub(in crate::core::app_state) fn parse_bool(raw: &str) -> Result<bool> {
...
pub(in crate::core::app_state) fn parse_list_value(raw: &str) -> Vec<String> {
