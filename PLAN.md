...
- 当前目标：基于架构基准继续收敛 Server 信任语义，去掉远程审批链，补齐剩余远程观测与入口统一。
...
- Server 模式运行时强制 `full_trust`，不再承载运行中审批。
- 远程角色收敛为 `controller / observer`，删除 `approver` 语义。
- 移除 Server 端审批 API 与审批事件，避免继续维护低价值复杂链路。
- 基于新的 Server 语义，继续补齐远程成本可见性与统一入口收尾。

### Phase 18（远程能力尾项收口）
- 为 Server 补充请求级 / 任务级 / 会话级成本查询接口，并保持会话可见范围控制。
- 继续收敛 GUI 与 Server 的入口抽象，减少本地/远程入口分叉。
- 清理历史文档中的远程审批语义，保证路线与实现一致。
