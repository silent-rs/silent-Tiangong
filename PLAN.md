...
| 0.8.3 | 浏览器缩放与开发体验优化 | ✅ |
| **0.9.0** | **嵌入式终端与消息列表导航交互** | **发布准备中** |

## 当前执行策略（2026-06-18）

- Phase 1~21 已完成。
- 当前主目标：基于最新主分支发布 0.9.0。
- 0.9.0 聚焦嵌入式终端基础能力（按对话独立 PTY + 显式 session_id 路由）、Agent 交互式终端操作能力（vi/nano/REPL + swap 处理）、消息列表滚动导航滑轨与活跃轮次自动展开/收起交互优化。
