...
- P0：由 LLM 层保证流式并行工具调用按原始序号完整累计，参数截断或解析失败可诊断、可有限恢复且不会向 Agent 暴露残缺调用。
