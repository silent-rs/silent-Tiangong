...
pub const PROMPT_SECTION: &str = "Subagent 工具使用规范：需要帮手时先用 create_agent 招募（同名成员会复用并延续其长期指令与记忆；默认创建后立即在当前会话激活，随后可直接派活）。招募通常选原生后端 agent_team（无需任何会话或命令参数，系统为其建立专属天工运行时会话并跨任务延续上下文），仅在需要关联某个已有会话（tiangong_session，需 session_id 或 session_query）或接入外部命令（cli，需 command）时选用其他后端。list_agents 查看全部持久 Subagent；向某个 Subagent 交办工作前必须已在当前会话激活（自动绑定当前 Workspace）。用户消息以「@成员名」开头或包含 @ 提及时，表示希望把内容定向交给该 Subagent：把去除 @ 标记后的内容用 send_agent_message 转达给对应成员（必要时先 activate_agent）。追问、补充背景、纠正方向用 send_agent_message；有明确目标和完成条件的正式工作用 submit_agent_task（立即返回，完成、阻塞、审批或失败会自动反馈到本会话，无需轮询等待）；派活与协作消息会自动携带发起方本轮用户消息中的附件（图片、文件等随消息送达成员，无需也无法手工指定附件路径）。跟踪进度用 list_agent_events / get_agent_run / get_agent_task；需要停止时用 interrupt_agent_run（可恢复现场）或 cancel_agent_run（终态）。读取与积累 Subagent 的长期记忆用 get_agent_memory / append_agent_memory；安排分工或判断进度时先用 list_pending_work 查看待处理工作与协作关系（谁在为谁执行、谁在等结果）。同一 Workspace 同时只有一个写入者，激活被拒时说明工作区被占用。成员之间可组成集群协同作业：执行中需要同伴（其他 Subagent 成员）协助、提供信息或接续工作时，用 send_agent_message 向该成员发送协作消息（对方完成后的回复会自动送回本会话）；收到「【Subagent 消息】来自成员「XX」」即同伴的协作请求，处理后在最终回复中给出结果即可，也可继续用 send_agent_message 与更多成员协作。Subagent 具备成长进化能力：完成一项工作后，先用 report_agent_result 向任务消息中的【回复地址】回报结果（纯消息投递，务必调用工具而不是只在回复文本里写结论）；随后把可复用经验（成功做法、踩坑、用户偏好，一行一条结论式）用 append_agent_memory（memory_name 指定 lessons.md）沉淀为经验记忆（后续运行优先注入）；确有稳定下来的新规则时，用 append_agent_instructions 追加进自己的长期指令——只记结论，不记流水。成员维护自己的工作状态：收到消息先用 load_workspace_state 加载本工作区状态，判断其意图（新工作/补充/协作结果/控制）后决定关联、拆分或调整（不默认当作新任务）；执行中用 update_workspace_state 的 task 文件维护当前工作与待办（task.md 内自行组织多项工作的分区）、plan 文件更新中长期规划、context 文件积累背景约定；收尾后保存下一步所需上下文——工作的含义由你维护，执行事实由系统记录。收到任务时注意【任务工作区】——那是发起会话的工作区，你的会话默认目录可能与它不同，所有文件操作和命令执行都必须使用基于该工作区的绝对路径。";
...
pub const TOOL_SPECS: &[(&str, &str, &str)] = &[
    ...
    (
        "send_agent_message",
        "向指定 Subagent 发送补充消息：追问、补充背景、纠正方向。该成员在本发起方若有进行中的工作，消息自动关联该工作（不新建执行，最终结果合并回报）；没有进行中的工作时才启动一次轻量消息往返。有明确目标和完成条件的正式工作请用 submit_agent_task。Subagent 成员之间也可用它发送协作消息（agent_id 支持成员 ID 或名称，目标无需在发起会话激活）。发起方本轮用户消息携带的附件会自动随消息送达目标成员。",
    ...
    (
        "submit_agent_task",
        "向指定 Subagent 提交有明确完成条件的正式任务。立即返回任务与运行编号，执行在后台进行；完成、阻塞、审批或失败会反馈到本会话。发起方本轮用户消息携带的附件会自动随任务消息送达成员。",
    ...
    (
        "report_agent_result",
        "成员向明确接收方投递回报消息（Subagent 专用：在自己的执行会话内调用）。接收方取任务消息中的【回复地址】；回报是纯消息投递——只负责把结果送达，不替你决定任务是否完成（工作含义由你和发起方各自判断）。完成收尾务必调用本工具回报，而不是只在回复文本里写结论。可选带运行标记（消息尾部的 r-短码）便于追踪。",
        r#"{"type":"object","properties":{"result":{"type":"string","description":"回报结果正文（结论、关键产出、未尽事项、遇到的阻塞）"},"to_session":{"type":"string","description":"接收方会话 ID（任务消息中的【回复地址】）"},"note":{"type":"string","description":"可选备注（产物位置、后续建议）"},"run_marker":{"type":"string","description":"可选运行标记（消息尾部的 r-短码），仅用于追踪"}},"required":["result","to_session"]}"#,
    ),
];
