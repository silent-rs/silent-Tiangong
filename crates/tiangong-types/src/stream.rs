...
pub enum StreamEvent {
    ...
    UserMessage {
        ...
        model_excluded: bool,
        /// 与本条消息同一次持久化确认产生的插件待投递快照，宿主必须原子同步。
        #[serde(
            default,
            alias = "pending_agent_deliveries",
            skip_serializing_if = "Vec::is_empty"
        )]
        pending_plugin_deliveries: Vec<crate::PendingPluginDelivery>,
    },
    ...
    SessionMessageUpsert {
        ...
        #[serde(default, skip_serializing_if = "Option::is_none")]
        #[serde(alias = "pending_agent_deliveries")]
        pending_plugin_deliveries: Option<Vec<crate::PendingPluginDelivery>>,
        /// 与投递快照同一原子状态变更中的已完成 ID；None 表示不修改。
        #[serde(default, skip_serializing_if = "Option::is_none")]
        completed_plugin_delivery_ids: Option<Vec<String>>,
        /// 与消息快照同一原子状态变更中的延迟注入列表；None 表示不修改。
        #[serde(default, skip_serializing_if = "Option::is_none")]
        deferred_tool_injections: Option<Vec<crate::DeferredToolInjection>>,
    },
    /// 尚未完成的插件持久投递快照，供宿主同步 Session 顶层状态。
    #[serde(alias = "pending_agent_deliveries_changed")]
    PendingPluginDeliveriesChanged {
        deliveries: Vec<crate::PendingPluginDelivery>,
        #[serde(default, skip_serializing_if = "Vec::is_empty")]
