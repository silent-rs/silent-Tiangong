...
use tiangong_core::core::{Plugin, ResolvedTurnModel, TiangongCore};
use tiangong_types::StreamEvent;

use crate::CoreManager;
use crate::core_manager::EnsuredCore;

/// 路由 Chat 槽位的默认模型（key + 端点）。
///
/// 路由项不在 models 注册表中时 key 退化为模型名——它只用于「是否发生
/// 切换」的比较，不需要是注册表 key。
fn default_chat_model(models: &tiangong_llm::models_config::ModelsConfig) -> ResolvedTurnModel {
...
impl CoreManager {
    ...
    /// Core 重建时的初始模型：按 Session.model_ref 与当前注册表解析。
    ///
    /// 用户选择失效（key 或 provider 已删）时**保留该引用**作为当前模型
    /// 状态，端点留空——这样下一次用户选择有效模型时，Manager 仍能识别
    /// 出发生了切换；真正发送前的解析会给出明确报错，不静默改成默认。
    fn resolve_initial_model(&self, session_id: &str) -> ResolvedTurnModel {
        let session_ref = self
            .load_session(session_id)
            .ok()
            .and_then(|session| session.model_ref);
        match session_ref {
            Some(key) => match self.resolve_turn_model(Some(&key)) {
                Ok(target) => target,
                // 引用失效：保留引用、端点留空，发送前解析会明确报错，
                // 用户重选有效模型时仍能识别出发生了切换。
                Err(_) => ResolvedTurnModel {
                    model_ref: key,
                    endpoint: tiangong_llm::ModelEndpoint::default(),
                },
            },
    ...
    /// 解析本轮的目标模型（`None` 表示跟随当前 Chat 默认）。
    ///
    /// `None` 只是选择策略，不能直接与 Core 当前模型比较——默认模型本身
    /// 会变化。必须先解析成实际目标再比较。
    pub fn resolve_turn_model(&self, model_ref: Option<&str>) -> Result<ResolvedTurnModel, String> {
        ...
        match model_ref.map(str::trim).filter(|key| !key.is_empty()) {
            Some(key) => {
                ...
                Ok(ResolvedTurnModel {
                    ...
                    endpoint: tiangong_llm::ModelEndpoint::from_resolved(
            ...
            None => {
                ...
                if target.endpoint.base_url.trim().is_empty()
                    || target.endpoint.model.trim().is_empty()
                {
    ...
    pub async fn switch_model_if_needed(
        ...
        if core.current_model_ref() == target.model_ref {
            return Ok(());
        }
        // 当前模型不可用（失效引用：Core 重建时保留了引用但端点为空）时
        // 跳过整理——用一个无法发请求的端点压缩必然失败，会把用户永久
        // 卡在失效状态。此时历史也从未被该模型处理过，直接切换即可。
        let current_usable = {
            let endpoint = core.current_endpoint();
        ...
        if current_usable {
            ...
            tracing::info!(
                session_id,
                current_model = %core.current_model_ref(),
                "当前模型不可用，跳过切换前的上下文整理"
            );
        }
