...
/// App ↔ Launcher 通信协议版本（唯一定义在库层，宿主与 bin 共同引用）。
const PROTOCOL_VERSION: u32 = tiangong_sandbox::LAUNCHER_PROTOCOL_VERSION;
/// 策略 Schema 版本（同上，库层唯一）。
const POLICY_SCHEMA: u32 = tiangong_sandbox::LAUNCHER_POLICY_SCHEMA;
/// 策略经此继承描述符传入。
#[cfg(unix)]
