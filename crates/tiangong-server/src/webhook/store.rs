...
impl WebhookStore {
    ...
    pub fn insert(&self, webhook: &Webhook) -> Result<()> {
        ...
        webhook.name = normalize_required_single_line("任务名称", &webhook.name)?;
        webhook.description = normalize_required_single_line("任务描述", &webhook.description)?;
...
/// 把多行/空白折叠为单行，并要求结果非空。
///
/// webhook 的「任务名称 / 任务描述」展示为单行字段，归一化避免卡片渲染跨行错位。
