...
use crate::execution::step_executor::execute_single_plan_step_with_execution_agent;
use crate::execution::{DynamicPlanStep, ExecutionStepReport, LlmOutputRecord};
use crate::model::{SingleProviderClient, TokenUsage};
use crate::planner::{PlanStepSource, PlanStepStatus, TaskPlan};
use crate::session::{Message, Session};
use crate::tool::{LocalToolExecutor, ToolResult};
...
where
    ...
    T: FnMut(&ToolResult),
    S: FnMut(&str),
    G: FnMut(&str, &crate::model::ModelStreamChunk),
{
...
fn append_dynamic_step(plan: &mut TaskPlan, plan_idx: usize, next_step: DynamicPlanStep) {
    ...
    item.execution_steps.push(crate::planner::PlanStep {
