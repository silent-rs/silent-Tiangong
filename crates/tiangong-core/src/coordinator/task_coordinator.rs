...
//! 汇总结果生成最终回复。
//!
//! 提供 sync 和 async 两种执行路径：
//! - `coordinate`：同步执行（桥接到 async）
//! - `coordinate_async`：直接 async 执行，Worker 通过 channel 接收命令
...
impl TaskCoordinator {
    ...
    /// async 协调执行（Worker 通过 channel 接收命令）
    pub async fn coordinate_async(
        &self,
        task: CoordinatorTask,
        session: &Session,
        stream_tx: &StdSender<StreamEvent>,
    ...
    pub async fn coordinate_split_async(
        ...
        if sub_tasks.len() <= 1 {
            let single_task = sub_tasks.into_iter().next().unwrap_or(task);
            return self.run_single_async(single_task, session, stream_tx).await;
        }
        ...
        let results = self.run_parallel_async(sub_tasks, session, stream_tx).await;
        self.merge_results(&task, results, stream_tx)
    }
    ...
    /// 单 Worker 执行（async，接收 AgentCommand channel）
    async fn run_single_async(
        &self,
        task: CoordinatorTask,
        session: &Session,
        stream_tx: &StdSender<StreamEvent>,
        ...
        let (_cmd_tx, cmd_rx) = Worker::command_channel();
        let worker = Worker::new(worker_context, self.engine.clone(), session.clone());
        let result = worker.run_async(stream_tx, cmd_rx).await;
    ...
    /// 并行执行多个子任务（同步，使用 std::thread::scope）
    fn run_parallel(
        &self,
    ...
    /// 并行执行多个子任务（async，使用 tokio::spawn）
    async fn run_parallel_async(
        ...
        for (i, task) in tasks.into_iter().enumerate() {
            ...
            let handle = tokio::task::spawn_blocking(move || {
                // spawn_blocking 内部创建独立的 tokio runtime 运行 async worker
                ...
                let (_cmd_tx, cmd_rx) = Worker::command_channel();
                let worker =
                    Worker::new(worker_context, engine, worker_session).with_multi_worker_mode();

                tracing::info!(worker_id, "Worker 开始执行 (async channel)");

                // spawn_blocking 内运行 async worker
                let result = rt.block_on(worker.run_async(&stream_tx, cmd_rx));

                tracing::info!(
                    worker_id,
                    success = result.success,
                    duration_ms = result.duration_ms,
                    "Worker 执行完成 (async channel)"
                );
                result
            });
        ...
        for handle in handles {
            match handle.await {
                ...
                Err(err) => {
                    tracing::error!("Worker 任务 join 失败: {err}");
                    results.push(WorkerResult {
                        worker_id: "unknown".to_string(),
                        result_text: String::new(),
                        success: false,
                        error: Some(format!("Worker 任务失败: {err}")),
                        usage: Default::default(),
                        duration_ms: 0,
                        llm_calls: Vec::new(),
                    });
