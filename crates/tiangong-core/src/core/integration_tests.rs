...
/// 切换模型后：下一轮先用**原模型**做交接压缩，成功后再用**新模型**继续执行。
///
/// 两个独立 mock server 分别扮演原模型与新模型，据此断言压缩请求确实落在
/// 原模型上（而不是仅看会话终态）。
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn model_switch_compresses_with_previous_model_then_uses_new_model() {
    use crate::config::core::CoreConfig;

    let (env, sid) = TestEnv::new("handoff-model");
    let old_model = MockServer::builder().start().await;
    let new_model = MockServer::builder().start().await;

    // 原模型：承担首轮回答与随后的交接压缩（同一 server 的路由须一次挂载）。
    let old_routes = mount_prompt_router(
        &old_model,
        vec![
            PromptRoute::new(
                "warmup",
                latest_user_contains("HANDOFF-EARLIER"),
                MockReply::sse(stream_text_chunks(&["原模型回答"])),
            ),
            PromptRoute::new(
                "handoff-compression",
                |request| request.is_compression(),
    ...
    .await;
    // 新模型：只负责压缩之后的正式回答。
    let new_routes = mount_prompt_router(
        &new_model,
        vec![PromptRoute::new(
            "post-handoff",
            latest_user_contains("HANDOFF-NEXT"),
            MockReply::sse(stream_text_chunks(&["新模型接续回答"])),
        )],
    )
    .await;

    let (core, mut events) = core_for(&env, &sid, &old_model.uri());
    // 先在原模型下积累一段真实历史（首轮定档指纹）。
    {
        let mut session = env.load_session(&sid);
        session.append_message(MessageRole::User, "HANDOFF-EARLIER");
        session.append_message(MessageRole::Assistant, "原模型的既有回答");
        session.try_persist_to_disk().unwrap();
    }
    // 首轮请求由原模型回答，确保指纹按原模型定档。
    send_message(&core, "warmup", "HANDOFF-EARLIER");
    events.wait_done_count(1);
    wait_idle(&sid).await;

    // 切换模型：只改端点，不动历史。
    core.replace_config(
        CoreConfig::builder()
            .with_chat(&new_model.uri(), "test-key", "new-model")
            .with_trust_mode(TrustMode::FullTrust)
            .build(),
    )
    .expect("切换模型应成功");

    send_message(&core, "after-switch", "HANDOFF-NEXT");
    events.wait_context_compressed(tiangong_types::stream::ContextCompressAction::Auto);
    events.wait_done_count(2);
    wait_idle(&sid).await;

    // 压缩必须发生在原模型上，新模型只收到压缩后的正式请求。
    old_routes["handoff-compression"].assert_hits(1);
    new_routes["post-handoff"].assert_hits(1);
    ...
    assert!(
        session.pending_config_handoff.is_none(),
        "交接成功后必须清除标记"
    );
    assert!(
        session
            .messages
            .iter()
            .any(|message| message.role == MessageRole::Notice
                && message.text_content().contains("上下文整理完成")),
        "应向消息列表写入交接完成的 notice"
    );
    core.shutdown_join().expect("关闭失败");
}

/// 交接压缩失败时：标记保留、指纹不提升，且明确告知新配置尚未生效。
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn failed_handoff_keeps_marker_for_retry() {
    use crate::config::core::CoreConfig;

    let (env, sid) = TestEnv::new("handoff-fail");
    let old_model = MockServer::builder().start().await;
    let new_model = MockServer::builder().start().await;
    ...
    let old_routes = mount_prompt_router(
        ...
        vec![
            PromptRoute::new(
                "warmup",
                latest_user_contains("HANDOFF-FAIL-EARLIER"),
                MockReply::sse(stream_text_chunks(&["原模型回答"])),
            ),
            // 原模型压缩失败：此时不得声称切换成功。
            PromptRoute::new(
                "compression-failure",
                |request| request.is_compression(),
                MockReply::error(400, "compression unavailable"),
            ),
        ],
    )
    .await;
    let new_routes = mount_prompt_router(
        ...
        vec![PromptRoute::new(
            "post-handoff",
            latest_user_contains("HANDOFF-FAIL-NEXT"),
            MockReply::sse(stream_text_chunks(&["压缩失败后仍继续回答"])),
        )],
    )
    .await;
    ...
    {
        let mut session = env.load_session(&sid);
        session.append_message(MessageRole::User, "HANDOFF-FAIL-EARLIER");
        session.append_message(MessageRole::Assistant, "既有回答");
        session.try_persist_to_disk().unwrap();
    }
    send_message(&core, "warmup", "HANDOFF-FAIL-EARLIER");
    events.wait_done_count(1);
    wait_idle(&sid).await;
    ...
    send_message(&core, "after-switch", "HANDOFF-FAIL-NEXT");
    events.wait_done_count(2);
    wait_idle(&sid).await;

    old_routes["compression-failure"].assert_hits(1);
    new_routes["post-handoff"].assert_hits(1);

    let session = env.load_session(&sid);
    assert!(
        session.pending_config_handoff.is_some(),
        "压缩失败必须保留标记，下一轮继续重试"
    );
    assert!(session.context_summary.is_none(), "失败时不得写入摘要");
    core.shutdown_join().expect("关闭失败");
}
