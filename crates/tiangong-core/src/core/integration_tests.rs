...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn cancel_during_candidate_seal_preserves_react_candidate_without_summary() {
    let (env, sid) = TestEnv::new("candidate-cancel");
    let candidate_marker = "CANDIDATE-SEAL-CANCEL-MARKER";
    let provider = ScriptedLlmProvider::new([ScriptedProviderCall::text_stream(
        &["CANDIDATE-SEAL-", "CANCEL-MARKER"],
        12,
        3,
    )]);
    let (core, mut events) = core_for_client(&env, &sid, provider.client());

    let mut seal = arm_seal(&sid);
    send_message(
        &core,
        "msg-candidate-cancel",
        "CANDIDATE-SEAL-CANCEL-PROMPT",
    );
    seal.wait_frozen();
    core.deliver(AgentInputKind::cancel())
        .expect("候选封口期间取消投递应成功");
    seal.release();

    assert_eq!(
        wait_turn_status(&env, &sid, "msg-candidate-cancel").await,
        TurnStatus::Cancelled
    );
    wait_idle(&sid).await;
    events.wait_cancelled();
    ...
    let user = session
        .messages
        .iter()
        .find(|message| message.id == "msg-candidate-cancel")
        .expect("取消对应的用户消息必须保留");
    assert_eq!(user.turn_status, Some(TurnStatus::Cancelled));
    let candidates = session
        ...
        .collect::<Vec<_>>();
    assert_eq!(candidates.len(), 1, "磁盘中应保留唯一候选答复");
    assert_eq!(
        candidates[0].phase,
        crate::session::MessagePhase::React,
        "取消后的候选答复必须保持 React 相位"
    );
    assert!(!session.messages.iter().any(|message| {
        message.text_content().contains(candidate_marker)
            && message.phase == crate::session::MessagePhase::Summary
    ...
    provider.assert_request_count_and_latest_user_contains(1, "CANDIDATE-SEAL-CANCEL-PROMPT");
    core.shutdown_join().expect("关闭失败");
}
