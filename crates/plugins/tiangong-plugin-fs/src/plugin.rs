...
//! 通过 [`Plugin::set_trust_mode`] 接收 core 注入的共享信任模式引用。工具规格与
...
/// `trust_mode` 由 core 在 register 前通过 `set_trust_mode` 注入（共享引用，值实时同步）。
#[derive(Default)]
pub struct FsPlugin {
    /// 当前会话工作目录（可变，由 core 注入）。
    workspace: RwLock<Option<PathBuf>>,
    /// 共享信任模式引用（set_trust_mode 时注入，FullTrust 时放宽路径校验）。
