...
//!
//! `recall_attempted`（本轮已回忆去重）与 `turn_count`（meta 反刍计数）是
//! **per-session 控制流状态**，按 `session_id` 索引隔离，避免多 session 并发
//! 互相覆盖。
//!
//! ## 写记忆业务
//!
...
impl Plugin for MemoryPlugin {
    ...
    fn on_turn_finished(&self, session: &mut Session, turn_start_idx: usize) {
        ...
        let rumination_handle = handle.clone();
        tokio::spawn(async move {
            rumination_handle
                .run_enhanced_micro_rumination(enhanced_result)
                .await;
        });
