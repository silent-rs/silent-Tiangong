//! 视频生成插件结构体定义与生命周期实现。
//!
//! [`GenerateVideoPlugin`] 通过 [`Plugin::register`] 从 [`RuntimeEngine`] 获取
//! [`ModelsConfig`]（克隆一份私有持有）并据此判定视频生成能力是否已配置，
...
pub struct GenerateVideoPlugin {
    ...
    /// 视频生成能力是否已配置（register 时据 LlmConfig/ModelsConfig 判定）。
...
impl Plugin for GenerateVideoPlugin {
    ...
    fn register(&self, engine: &RuntimeEngine) {
        ...
        self.has_video
            .store(media::has_video_generation(&models, llm), Ordering::Relaxed);
