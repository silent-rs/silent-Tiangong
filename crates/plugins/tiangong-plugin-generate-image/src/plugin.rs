...
//! [`ModelsConfig`] 路由解析图片生成对应的 [`ModelEndpoint`] 私有持有，供 handler 调用
//! media facade。端点不再经 `LlmConfig` 字段中转，直接从模型配置路由转换获得。
...
pub struct GenerateImagePlugin {
    ...
    /// 从 ModelsConfig 路由解析的图片生成端点，供 handler 调用 media facade。
