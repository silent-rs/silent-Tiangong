...
//!
//! 对齐 Memory / MCP sidecar：`127.0.0.1:0` 动态端口监听、本地 endpoint 文件发现、
...
/// 启动 Index IPC bridge，将本地 [`IndexService`] 暴露为 TCP loopback 服务。
pub fn spawn_index_bridge(
    service: impl Into<String>,
    service_obj: IndexService,
) -> Result<IpcBridge> {
    ...
    let join_handle = thread::Builder::new()
        .name(format!("index-ipc-bridge-{service}"))
        .spawn(move || {
            let rt = tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()
                .expect("Index IPC runtime 构建失败");
            ...
            local.block_on(&rt, async move {
                ...
                if let Err(err) = run_index_bridge(service, server, service_obj, shutdown_rx).await
                {
                    tracing::warn!("Index IPC bridge 退出异常: {}", err);
                }
            });
        })
        .with_context(|| "创建 Index IPC bridge 线程失败")?;

    ready_rx
        .recv()
        .with_context(|| "等待 Index IPC bridge 就绪失败")??;
...
async fn run_index_bridge(
                    ...
                    Ok(connection) => {
                        ...
                        tokio::spawn(async move {
                            if let Err(err) = serve_connection(connection, service_obj).await {
                                tracing::debug!("Index IPC 连接结束: {}", err);
                            }
                        });
                    }
                    Err(err) => {
                        tracing::warn!("接受 Index IPC 客户端失败: {}", err);
    ...
    tracing::info!(service = %service, "Index IPC bridge 已关闭");
    Ok(())
}

async fn serve_connection(
    mut connection: IpcConnection,
    service_obj: Arc<IndexService>,
) -> Result<()> {
    loop {
        ...
        let payload =
            serde_json::to_value(plugin_response).with_context(|| "序列化 Index 响应失败")?;
        connection
...
fn endpoint_path(service: &str) -> Result<PathBuf> {
    ...
    Ok(index_runtime_dir()?.join(format!("{service}.json")))
}

fn index_runtime_dir() -> Result<PathBuf> {
