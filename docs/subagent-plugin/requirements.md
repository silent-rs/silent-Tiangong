...
### 0.2.24 追加（用户裁定：工作状态下沉到成员数据目录）

**双类状态分离**：

| 类型 | 示例 | 维护者 |
|---|---|---|
| 工作状态 | 任务目标、计划、待办、依赖、阻塞原因、成果、下一步 | Subagent（成员理解与维护） |
| 执行事实 | 消息送达、进程退出、中断发送、连接状态 | sidecar／执行系统 |

成员可判断「任务完成待验收」，系统只记录「回报已送达」；成员可决定取消工作，系统不得因此把运行中的进程记成已退出——模型判断不取代真实执行事件。

**成员数据目录（workspace 域组织）**：

```text
agents/<agent-id>/
├── instructions.md        # 成员长期规则
├── memory/                # 跨工作区可复用经验（lessons/notes）
└── workspaces/<workspace-id>/
    ├── workspace.json     # 工作区稳定身份 + 本机路径（路径非身份，跨设备兼容）
    ├── plan.md            # 成员在该工作区的工作规划
    ├── context.md         # 工作背景与关键约定
    └── tasks/             # 任务笔记
```

- workspace-id 为稳定身份（路径仅是某设备上的位置）；多个主 Agent 可在同一工作区派活——任务保留发起者、回报接收者与关联消息，不因 workspace 相同自动合并；
- **成员经统一附加 prompt 维护工作状态**：收到消息加载对应工作区状态 → 判断消息意图（新工作/补充/协作结果/控制）→ 决定关联、拆分或调整（不默认关联最新）→ 执行中更新进展阻塞 → 回报并保存下一步上下文 → 提炼跨任务经验到 memory；状态写入有序进行（多后台执行并行但不同时覆盖同一任务清单）；
- 状态存取经统一内部工具，sidecar 只提供安全可靠的存取与查询视图，不理解和决定业务内容。

**双层规划**：主 Agent 维护整体调度规划、跨成员依赖与验收；Subagent 维护分配给自己的工作、局部拆分与进展——「成果已提交」（成员）与「验收通过」（主 Agent）职责不同不重复；项目既有 PLAN/TODO 等正式记录保持对应范围权威，成员私有状态不与之矛盾。

**0.2.24 落地内容（骨架阶段）**：workspaces/&lt;workspace-id&gt;/ 目录与稳定身份（路径 canonical 归一、跨别名同身份）；成员状态三工具——load_workspace_state（读 plan/context/任务笔记）、update_workspace_state（写 plan/context，仅成员自己的执行会话）、save_task_note（任务笔记独立文件互不覆盖）；投递正文注入工作区状态摘要（截断保护）；提示词加入成员状态维护规则（收到消息先加载、判断意图、不默认新任务、执行中更新、收尾保存上下文）。执行事实层（归因/送达/退出/占用）保持不变——收敛项待成员状态经真实使用成熟后分阶段退出。

**0.2.25 追加（观测面）**：管理页成员详情新增「工作区状态（成员自维护）」区块——按工作区显示 plan/context 与任务笔记清单；MCP 暴露 load_workspace_state（映射 ui_list_workspace_states 全工作区列举——外部调度者查询成员进展，写入仍限成员自己）。

**0.2.26 追加（用户裁定：十轮复审——稳定任务编号与入口收敛）**

- **稳定任务编号**：create_workspace_task / update_workspace_task / read_workspace_task / list_workspace_tasks 四工具——系统分配 scru128 编号（t- 前缀），文件身份不受名称清洗影响；任务名称只是内容中的显示字段；写入限成员自己的执行会话。save_task_note 保留为早期骨架（标注过渡）；
- **入口存在校验**：工作区状态公共入口统一验证成员存在——不存在时不创建任何目录直接报错（消除幽灵目录）；
- **load_workspace_state 缺省成员**：agent_id 改为可选——成员会话缺省解析为自己，主会话/非成员会话须显式指定，缺失时明确失败；
...
**0.2.29 追加（用户裁定：创建后所有会话自动可用，取消手动激活策略）**

- **自动激活**：send_agent_message / submit_agent_task 的主会话路径在当前会话未激活时自动建立激活（默认只读策略、绑定当前会话工作区，auto- 前缀记录），不重复建——Subagent 创建后所有会话自动可用，@成员或派活即被调度，无需手动激活步骤；
- 管理页「提交任务」按钮与消息输入框不再要求本会话已激活；手动「激活/停用」按钮保留（用于显式切换工作区策略）。

**0.2.30 追加（用户裁定：工作区随发起会话）**
...
**0.2.37 追加（用户裁定：会话后端取消改调度标记，撤硬取消调用）**
- `cancel_agent_run` / 管理页停止对会话后端运行改为**纯调度标记**：立即把运行置 Cancelled 终态、任务取消并释放占用，再尽力投递取消通知（消息标记，投递失败不影响取消成立）——不调用 Server 取消端点、不等执行侧收尾；
- 动因：成员执行会话按 workspace 固定共享，当前轮次可能正执行**其他发起方**的运行，硬取消会话轮次会跨运行误伤（调度记录未按会话隔离的根源）；标记后执行侧轮次自然收尾，迟到结果只记事件（run 已终态，归因链不再改写结论）；
- CLI 后端取消保持实际终止（托管进程独占，无共享问题）；Server 取消端点保留（前端停止按钮等宿主能力在用，插件不再调用）。

**0.2.38 追加（面板调度记录按会话隔离）**
- 管理页 `ui_state_snapshot` 的活动任务 / 最近运行 / 最近事件按请求会话过滤（运行含 origin_session 协作发起命中；成员列表保持全局——身份与激活管理不分会话）；会话上下文缺失（空 session_id）时保持全局视图兜底。修复：在 A 会话面板能看到 B 会话（其他工作区）的调度记录。
