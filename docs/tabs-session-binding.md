...
- Agent 创建终端标签后，隐藏页面必须先完成输出订阅与精确 PTY 附着，随后才能开始执行命令；历史回放只可用于同一存活 PTY 的异常恢复。
- 新建标签没有同编号存活 PTY 时必须创建全新 PTY，不得回放该会话内其他终端的历史。
...
- 多个标签并行挂载时，早到的集合可能暂时不完整；sidecar 只能回收曾被前端确认存在、后来从集合消失的终端，不得回收尚未完成挂载的新终端。
- 标签切换、会话切换、拓展区隐藏和普通容器卸载不得从存活集合移除终端。
- sidecar 收到集合后立即结束同一会话中曾被前端确认、当前已不在集合内的 PTY，并从运行表移除；不增加周期扫描任务。
...
- [x] Agent 创建的隐藏终端会在命令执行前完成订阅和精确附着。
- [x] 新建终端不会显示已关闭终端或同会话其他终端的历史。
- [x] 多终端并发新建时不会误回收尚未完成挂载的终端。
