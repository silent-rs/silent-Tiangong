...
3. **交互类能力无统一通道**：操作审核（审批）、agent 需要用户选择/填写等「需要人参与」的流程，目前散落在 Core 的 `WaitingApproval` 状态、工具流水线等各处，没有统一的 UI 承载和扩展点。
...
**现状问题**：审批目前是 Core 内 `WaitingApproval` 状态 + 工具流水线内的阻塞逻辑，审批 UI 与审批策略耦合在核心流程里，三方既无法自定义审批界面，也无法新增「需要人确认」的自定义操作。
...
**内置迁移**：Core 的 `WaitingApproval` 状态保留为「审批进行中」的底层标记，但审批 UI 与策略移到审批接缝；默认审批插件替换现有审批弹窗。
...
现状：Core `WaitingApproval` 状态 + 内置审批弹窗 + 工具风险分级。

迁移：

- 默认审批 UI 改写为官方「审批」插件（`global.*` 或会话区容器 + 审批接缝处理器）。
- Core 保留审批状态机与 fail-closed 策略，UI 与路由策略移到审批接缝（见 7.5）。
...
| M4 | 交互接缝：审批 + 交互落地 | 审批/表单可被三方替换，agent 填写选择走统一通道 |
