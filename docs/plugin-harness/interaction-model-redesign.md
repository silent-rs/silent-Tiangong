...
- 工具调用到达时若无插件 UI 接应（无存活 `tool.requested` 订阅者），经 `app.open`
  桥接原语请求打开该插件的 App 实例（通用能力，官方与三方插件一致；插件需有
  `extension.tab` 贡献）。实例挂载完成订阅后由重放机制继续执行本次调用。同一插件
  短时间内重复请求会被冷却窗口抑制；后台会话的工具调用不弹面板；
- `app.close` 原语按插件关闭其 App 实例（Agent/插件在必要时收起对应 tab）。插件
  声明 `app.use` 权限后可经 `bridge.call` 主动打开/关闭自己的 App；宿主内部使用
  不经权限校验；
