...
4. 单例/多实例打开规则明确：单例 App 重复打开聚焦已有实例；多实例 App 每次主动新建时创建实例；是否自动展开拓展区必须由声明 `extension.tab` 且具有 `app.use` 权限的 App 插件通过公共 SDK 设置，宿主容器不得解释具体工具参数，也不得按插件 ID 读取私有快照判断后台实例。Agent 后台读取网页时必须先建立可供用户观察的浏览器 App 页面，浏览器插件自行用 `web_fetch.open` 决定是否自动展开，用户手动进入浏览器时聚焦 Agent 正在操作的同一页面；切换会话时实例按会话隔离。
...
14. 浏览器页面达到可读状态后必须立即返回内容，不得因仍有长连接或非关键资源而固定等待 30 秒截止时间；终端插件执行 `run_command` / `run_shell` 时必须自动打开对应终端标签，在该标签的正常交互 shell 中执行并保留提示符，命令完成后向 Agent 返回真实输出与退出状态；多个前后台实例不得重复执行同一调用，也不得把已经退出的临时进程恢复为终端标签。
15. MCP 动态工具名为满足模型函数命名限制而转换后，执行路径必须通过稳定映射还原真实 server/tool 名称；管理页显示连接正常的 server 不得因连字符、空格等名称字符在调用时被误判为不存在或未启用。
16. 插件卸载后不得被事务暂存目录、回滚目录或其他非正式安装目录重新发现，重启 App 后仍保持未安装；插件目录仅在目录名与清单 ID 一致时计入已安装状态。线上版本仅在语义版本严格高于本地正式安装版本时显示并执行升级，本地与线上版本一致或本地更新时不得显示升级入口。

### 插件进程随宿主退出（#513）

- stdio 插件在应用正常退出、崩溃或被强制结束后必须退出，不得长期残留为孤儿进程。
- 宿主关闭输入管道必须能够使插件结束；真实沙箱中的空闲与执行中场景均需验证。
- 插件创建的后台进程也必须随本次调用或宿主退出被清理；沙箱内只允许清理自身及后代，不得因此取得终止宿主或其他插件的权限。
- TCP 共享服务的跨应用存活语义不在本次修复范围内。
