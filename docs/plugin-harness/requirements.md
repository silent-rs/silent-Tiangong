...
12. 默认截图输入插件使用 Vue 3 虚拟 DOM 与 Vite 单文件构建，继续挂载在 `session.input-action` 的 Shadow 容器中；点击后仍通过通用宿主桥接发起区域截图并将 PNG 加入当前输入草稿，不自动发送。插件必须直接继承 App 主题变量、使用作用域样式、在宿主卸载时释放 Vue 实例，并可组装为不含源码和依赖目录的本地导入包。
