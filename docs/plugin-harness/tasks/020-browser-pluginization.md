...
- 管理界面（地址栏/导航/标签条）在插件 UI（shadow 容器）；
...
| `browser-handler` 插件 UI | 地址栏、导航、标签条、位置同步（内容区矩形 → `webview.show/setPosition`） | 本任务实现 |
...
- [x] 插件 UI 最小闭环：SDK 桥接入、地址栏导航、后退/前进/刷新、标签条（新建/切换/关闭）、webview 位置跟随内容区（ResizeObserver + 窗口事件）、面板卸载隐藏（会话保留、重开恢复）。
...
- [x] 插件 UI 第二阶段：以迁移前 `BrowserPanel` 为基准完整对齐标签栏、工具栏、地址栏、明暗主题和窄屏布局。
...
- `yarn package` 本地签名打包，「设置 → 插件管理 → 导入本地插件」实测：打开浏览器 tab、地址栏导航、多标签管理、位置跟随、关闭重开恢复。
- 与 `main` 分支迁移前的 `BrowserPanel` 逐项对照，桌面和窄屏下不出现功能、状态反馈或视觉层级回退。

2026-08-20：浏览器插件与宿主前端构建通过，前端 210 项现有检查通过，Rust 工作区检查及浏览器模块严格检查通过；桌面、360px 窄屏、明暗主题与主要操作流程可视化验收通过。
