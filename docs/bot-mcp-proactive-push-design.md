...
> 状态：三端实现及自动生命周期已完成，QQ、微信长期发送能力待真实验证
> 日期：2026-07-24
...
Bot 注册完成后并不会自动知道收件人。首期要求目标移动端会话先向 Bot 发送至少一条消息，让 Bot 获得平台会话标识、自动加入目标清单并授权该目标。清单支持多个目标，由 Bot 持久维护；移动端控制页只展示授权记录并允许删除，不提供授权开关，也不要求用户在首次使用前再次选择。

首期让每个 Bot 制品增加 `--mcp` stdio 模式，并通过 `--mcp generate` 输出普通 MCP 注册配置。Desktop 保留 `bot_register_mcp` 命令，前端启动支持 MCP 的 Bot 后自动调用，应用重启恢复运行中的 Bot 时也会补充检查；用户停止 Bot 或删除配置时自动注销。不支持 MCP 的旧 Bot 不触发该流程。现有 MCP 客户端已经支持 stdio，通知推送频率通常较低，单次启动进程的开销可以接受；HTTP 模式会额外引入端口发现和本地鉴权，当前没有必要。
...
7. Bot 注册信息本身不包含移动端收件人，必须先由移动端发来消息完成目标发现。

## 3. 目标与非目标

### 3.1 目标

- 支持 Agent 在一次有效执行中主动调用指定 Bot，向 Bot 清单中的一个或多个已授权目标分别发送文本、图片或文件。
- Bot 自己持有平台凭证、目标地址和平台发送逻辑，天工主程序不感知飞书、QQ、微信专属字段。
- 支持前端启动 Bot 后自动生成并注册普通 MCP，停止后自动注销。
...
- 天工将 Bot 与其生成的普通 MCP 按固定名称关联，并随 Bot 启动、停止和删除同步注册状态；主程序不解释平台地址与上下文令牌。
...
安装时缓存完整描述信息；现有只缓存配置字段的 `schema.json` 继续保留，新增完整的 `description.json`，避免破坏旧数据。未声明 `mcp` 的旧 Bot 不执行自动注册，也不显示推送目标入口。
...
只读工具，返回 Bot 自动维护的当前授权目标，不返回真实平台 ID、凭证或上下文令牌。
...
有外部副作用的工具只接受 Bot 生成的 `target_id`，不接受原始 `chat_id`、OpenID 或用户 ID。Agent 可以根据 `list_push_targets` 选择目标，并通过多次调用向不同目标发送。
...
目标清单由 Bot 维护，移动端发消息本身视为该会话对 Bot 的显式授权：

1. 每次收到移动端消息时按平台会话新增或更新目标、最近使用时间和回复上下文，并自动启用该目标。
2. 移动端控制页展示当前授权记录，每项只提供删除操作；删除后目标立即从 Bot 清单移除，同一会话之后再次发来消息时重新发现并自动授权。
3. `list_push_targets` 只返回当前已授权目标，发送工具只接受其中的 `target_id`，不接受任意平台地址，也不提供广播调用。

目标文件及平台专属字段只由 Bot 读写。Desktop 只通过通用 `--push-target-list` 和 `--push-target-delete` 命令展示或删除授权记录，不维护平台会话清单。

首期允许 Agent 按清单逐个发送，不提供单次广播或“一键全部发送”。

## 7. MCP 注册与生命周期

当 Bot 声明 `mcp` 能力时，前端启动 Bot 成功后调用保留的 `bot_register_mcp` 命令，由 Desktop 执行 `bot --mcp generate`、完成安全校验并调用现有 MCP 插件的普通注册接口。用户停止 Bot 或删除 Bot 配置时，Desktop 自动注销匹配的 MCP；应用重启自动恢复运行中的 Bot 时也会重新确认注册状态。

- MCP server 名称固定为 `bot-<bot-id>`。
- 生成配置与已有 MCP 同名但内容不同时拒绝覆盖。
- 注册结果写入现有 `mcp.json`，但生命周期由前端 Bot 启停流程负责，不再提供手工快速注册按钮。
- 已存在同名且内容一致的配置时直接复用并启用；同名但内容不一致时拒绝覆盖，停止时也不删除该配置。
- 前端自动注册失败时停止刚启动的 Bot；自动注销失败时不继续停止，避免 Bot 与 MCP 状态分离。
...
- 只有移动端主动联系后由 Bot 自动授权、且未被用户删除的目标才能被发送工具访问。
...
- 工具禁止广播；群聊与私聊都只有在对应会话主动联系 Bot 后才进入授权清单，用户可随时删除。
...
4. 增加通用目标存储、授权清单和删除命令。
5. 增加 `--mcp` stdio 入口及文本、图片、文件发送工具。

### 阶段三：Desktop 注册

1. 保留 `bot_register_mcp`，由前端在 Bot 启动成功后按能力声明自动调用，停止和删除时自动注销。
2. 复用现有普通 MCP 注册、注销与健康检查。
3. 移除移动端控制页的 MCP 手工注册按钮和授权开关，只保留目标授权展示和删除入口。
...
1. 为 QQ 和微信接入目标发现、Bot 自维护清单、MCP 配置生成与 stdio 服务。
...
- 支持能力的 Bot 从前端启动成功后自动出现对应 MCP 工具，停止或删除后自动移除。
- 新注册 Bot 在尚未收到移动端消息时没有推送目标，移动端发来消息后由 Bot 自动加入清单。
- Agent 可以读取 Bot 维护的多目标授权清单，并按目标编号分别发送；应用可以展示或删除授权记录，但不负责发现目标。
...
- 应用重启后运行中的 Bot 保持 MCP 注册；用户停止 Bot 或删除配置后对应 MCP 不再存在。
...
| 发消息即自动授权目标 | Agent 可能向曾联系 Bot 的非预期会话发送 | 应用可查看和删除授权记录，发送仍受工具执行权限约束且不提供广播 |

## 14. 最终建议

采用 Bot MCP 方案，但把它定义为“Bot 出站工具协议”，不要定义为“主动推送触发协议”。首期以 `bot --mcp generate` 自动注册、`bot --mcp` stdio、Bot 自维护多目标清单、文本、图片和文件发送及飞书真实闭环为边界。QQ 与微信先完成平台能力验证，再决定提供长期主动推送还是仅提供回复窗口内发送。
