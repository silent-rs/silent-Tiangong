...
- App 必须根据固定能力快照生成明确处理方式：对话模型支持多模态时图片使用 `InlineImage`；否则在附件分析插件可用时使用 `AnalyzeWithPlugin`；两者都不可用时拒绝该图片并保留草稿；文档及其他普通文件使用 `FileReference`；音视频保存为稳定引用，并明确标记对应处理能力是否可用。
- 准备完成的用户消息必须区分稳定持久内容与本轮运行内容。会话只保存附件 ID、本地路径、原始文件名、MIME、大小、类型和处理方式，不得保存大段 base64；本轮内联图片数据只进入 Agent Loop 请求。后续轮次需要再次发送历史 `InlineImage` 时，Provider 只能按已保存标记机械地从本地引用重新编码，不得重新进行能力决策。
