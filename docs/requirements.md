...
- 会话持久数据除对话消息外，还必须保留准确恢复所需的摘要边界、会话配置、用量统计、任务记录和已接收未完成工作；只有能由当前配置无损重建的缓存和派生值才能留在 Core 运行时且不落盘。
...
- Bot 实例 ID 必须为 1～64 位小写英文和数字，首位必须为英文字母；该 ID 同时用于配置主键和运行目录。后端所有入口必须统一校验，非法本地目录不得扫描、执行或写入，Windows 可执行文件后缀由主程序固定处理且不属于 Bot ID。
- Desktop 的移动端控制配置必须支持 bot schema 声明的扫码能力，并展示 bot 返回的授权二维码与状态。
- 平台专属扫码能力必须由对应 bot 制品提供；主程序只通过统一的子进程命令发起扫码和轮询，不得内置飞书域名、请求参数或状态码。
- 飞书 bot 的扫码配置必须使用飞书应用注册协议生成授权二维码，按服务端返回的间隔轮询，并明确处理等待、放慢轮询、拒绝、过期和授权失败状态。
- 飞书扫码成功后，App ID 与 App Secret 必须由飞书 bot 自行保存和读取；主程序只接收成功状态，不得接收、回填或持久化扫码所得凭证。
- 天工服务地址和天工认证 Token 属于主程序运行配置，不得出现在 bot schema 或 bot 配置表单中；启动 bot 时必须根据当前 Server 配置自动注入。
- Desktop 的 bot 列表不得单独提供启用开关；绿色三角用于启动并记录为随天工自动运行，红色实心方块用于停止并取消自动运行，两个按钮不显示外围留白。
- Desktop 开始 bot 扫码配置后必须隐藏名称、手工输入字段和保存按钮；扫码成功且 bot 自行处理完配置后，必须自动保存 bot 条目并关闭配置窗口。手工配置保存成功后也必须自动关闭配置窗口。
- Desktop 的已配置 bot 必须提供日志查看按钮；日志窗口展示该 bot 当前日志的最近内容，支持手动刷新，并明确展示空日志与读取失败状态。日志读取必须限制返回大小，不能一次性载入全部轮转日志。
...
- 用户输入侧必须支持图片、文档、音频和视频附件；入口是否可用不能只由独立 `multimodal` routing 决定，具体处理方式由目标会话在发送开始时捕获的对话模型与插件能力快照决定。
- 每个会话必须独立保存文字草稿、附件列表、发送状态和单调递增的修订版本。新建但尚未落盘的草稿会话必须使用稳定临时 ID；创建真实会话后，草稿、附件和版本信息必须完整迁移。
- 普通发送、执行中追加和编辑重发必须在任何异步附件处理前固定 `target_session_id`、草稿快照、修订版本和能力快照；后续不得重新读取全局活动会话作为投递目标。
- Desktop 可以提交本地路径或受限大小的 data URL 作为原始附件来源；任一附件超过 50MB 时必须停止整条消息准备并保留草稿。原始附件不得进入 Core。
- Desktop 与 Server 必须复用同一个 AttachmentStore。用户上传的全部附件由宿主 App 保存到 `~/.tiangong/media/`，并保持用户选择顺序；Core 和 Agent 插件不得下载、复制、解码或重新归档用户附件。
- AttachmentStore 必须区分本轮新建文件与已复用文件。任一附件保存失败时整条消息不得投递；后续规划、持久化或投递失败时只删除本轮新建文件，不得删除已存在或被其他消息引用的附件。
- App 必须根据固定能力快照完成全部附件决策和最终消息装配：对话模型支持多模态时生成可直接发送的图片内容块；否则在附件分析插件可用时生成稳定资源引用和仅供模型使用的动态说明；两者都不可用时拒绝该图片并保留草稿；文档、音频和视频同样由 App 生成稳定资源引用及对应说明。处理策略、插件名和能力状态不得进入 Core 合同或会话持久化数据。
- 准备完成的用户消息必须由通用最终内容块组成，至少区分用户可见文本、仅供模型使用的文本、可直接发送的图片和不直接发送给模型的稳定资源引用。会话只持久化资源 ID、本地路径、原始文件名、MIME、大小和类型，不得保存大段 base64；本轮图片数据随图片块进入 Agent Loop，但序列化时必须省略。后续轮次图片数据缺失时，Provider 只允许根据图片块中的本地引用机械读取并编码，不得重新判断模型、插件或附件策略。
- 历史图片的编码优化不得改变上述持久化边界；只有在性能数据证明存在瓶颈后，才可引入不落盘、按总字节限制并能感知本地文件变化的进程内缓存；缓存未命中或本地文件缺失时必须保持现有行为。
- Core 必须直接接收宿主提供的已就绪内容块列表并原样写入消息和上下文，不得再引入单字段消息包装，也不得生成附件提示、分配附件序号、解释能力状态或硬编码任何附件处理插件名。Provider 只能把文本和图片块机械映射为模型请求，并忽略稳定资源引用。
- 宿主生成的动态附件说明必须随消息持久化并携带准确的消息 ID、资源序号和本地路径，但不得作为用户可见正文展示、复制或参与编辑；插件静态使用说明应由插件自身贡献。
- 旧版 `media` 和本开发分支曾写入的附件消息必须保持可读取，并在进入新发送链路时转换为通用内容块；兼容逻辑不得重新进入新消息的 Core 输入准备路径。
- 发送成功后只能清理目标会话的草稿和附件，并且仅在当前修订版本仍等于发送开始版本时清理；等待期间产生的新输入必须保留。发送失败时只恢复目标会话发送状态，并保留文字和附件供重试。
- 编辑重发必须先完成最终有效性校验，再终止旧任务；有效旧附件直接复用，只保存新增附件，混合新旧附件保持原始顺序，失败时不得影响原附件。
- 后端必须按会话持久化草稿，并兼容旧版全局 `input_draft`：首次加载后将旧草稿归入当时的活动会话。
- 用户在消息文字中直接输入本地图片路径或普通网页 URL 时，不得自动视为上传附件；只有经 App 附件准备管线产生的稳定附件引用才使用上述处理方式。
...
#### 上下文管理
- 上下文压缩策略以对话模型的有效 `context_window` 不低于 200,000 tokens 为前提。
- 自动压缩阈值必须从当前模型上下文上限派生：为压缩输出保留上下文的 5%，并额外保留 `max(context_window * 1%, 4096)` 安全余量；用户配置只能让压缩更早触发，不能突破该安全上限。
...
- 压缩请求的 `max_output_tokens` 不得超过上下文上限的 5%，且必须根据实际剩余空间动态收紧；压缩提示词必须声明同一预算并要求模型在预算内主动结束。
- GUI 必须展示当前请求上下文 tokens、触发上下文压缩的进度条和当前会话累计总 tokens。
- 所有会话相关 LLM 请求产生的 token 消耗都必须进入会话统计，至少覆盖主对话请求、工具调用决策、附件解析、记忆召回和上下文压缩摘要请求。
- 自动压缩使用 LLM 将当前摘要边界后的历史折叠为新摘要；进行中任务必须通过一条持久化到 Session 的 `CompressedResume` User 消息续接。该消息必须位于 system prompt 后第一条、始终对模型可见，并在下一次压缩后仅在模型上下文中由新续接消息取代；Session 继续保留完整原始对话，GUI 不得展示、搜索或编辑续接消息。
- 压缩输出必须包含非空摘要；Provider 因最大输出长度停止、摘要为空或需要继续执行时缺少任务状态，均不得推进摘要边界。
- 自动压缩和手动压缩必须响应 Cancel/Shutdown；压缩请求只在副本上执行，成功持久化后才能提交真实 Session，失败或取消不得留下半更新。
- Provider 明确返回上下文超限时，强制压缩必须只折叠较早的完整历史区间，并保留最近一条模型可见消息；若最新消息是 Tool 结果，则必须连同对应的 Assistant 工具调用保留完整批次。压缩请求不得再次携带该保留区间，成功后必须通过新的续接消息与保留区间衔接。
- LLM 摘要失败时必须保留原上下文并发布失败反馈，不得推进摘要边界。
...
- GUI 必须在上下文压缩开始、成功、失败和取消时给用户可见反馈，避免长对话中压缩过程表现为无响应。
...
- 每个对话会话必须持久化自己的工作区 Tab 集合、跨插件排列顺序和当前活跃 Tab，切换会话时恢复该会话的浏览器/终端 Tab 集合。
- 工作区 Tab 不得写入 Core Session。浏览器与终端插件必须分别按会话持久化自己拥有的 Tab 元数据；Desktop 只允许保存由 `{kind, id}` 组成的跨插件排列顺序与当前活跃引用，不得复制插件标题、地址或运行状态。首次升级时必须在 Core 恢复前迁移旧 Session 中的 `tabs` 与 `active_tab_id`，不得因 Core 重写会话文件丢失恢复信息。
- 会话 Tab 只持久化可序列化元数据，不持久化 PTY 进程或 WebView 实例；恢复会话时按记录懒创建运行实例。
- 终端必须支持同一会话内多个独立 Tab，每个终端 Tab 拥有独立 PTY、cwd 和持久化日志。
- 浏览器会话切换时必须隐藏旧会话的 WebView、停止旧会话轮询但保留其运行状态，并按新会话 Tab 记录恢复浏览器 Tab 元数据与活跃 Tab。
...
- Agent 主循环调用主模型生成回复或工具调用时必须显式设置输出上限；当前默认值为 32,768 tokens。上下文窗口与最大输出能力必须分别处理，压缩等特殊请求可以在模型能力和剩余上下文允许范围内单独覆盖该值。
