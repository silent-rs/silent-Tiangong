...
状态：草案（2026-08-22）；修订（2026-08-27）：L4 放开开关明确不开发；
统一签名信任落地后 L3 本地确认演进为「本机用户密钥自动签名」，官方信任根
唯一化（`TIANGONG_PLUGIN_PUBKEY_B64` 覆盖通道已删除）——见 D1/D3/D4 修订标注。
...
| L4 放开开关 | ~~全局一次性高危动作~~ **不开发**（见 D4 修订） | 无（用户自担） | 插件开发者本地调试 |

### D1. L1 保持现状
官方 minisign 签名通道不变：`publisher == "tiangong-official"`、内置公钥。

> 修订（2026-08-27）：官方信任根唯一且不可配置——`TIANGONG_PLUGIN_PUBKEY_B64`
> 环境变量覆盖通道已删除（测试密钥不得伪装官方身份）；发布链以
> `verify-official-release` 强制校验发布私钥与内置公钥匹配。
...
> **决策修订（2026-08-27）：明确不开发。** 统一签名信任体系已为原设想场景
> 提供安全替代路径——开发者调试走本机用户密钥签名（创作链自动签名或
> `xtask sign-plugin` 直签）、三方联调走公钥导入 + 本地导入安装、CI 走测试
> 密钥局部验证（不可伪装官方）。一个全局跳过验证的开关会削弱刚建立的
> 信任边界，且与「官方信任根不可配置」原则直接冲突；若未来出现「秒级
> 试跑完全未签名脚本」的真实需求，应以显式开发模式配置单独评估，不复活
> 本节方案。以下原文保留作历史设计记录。
