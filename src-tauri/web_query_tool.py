...
class WebQueryTool:
    ...
    def search_glm_requirements(self) -> Dict:
        ...
        results = {"source": "GitHub - THUDM/GLM-4", "models": [], "queries": []}
        ...
        try:
            ...
            results["queries"].append(
                {"query": "HuggingFace: GLM-4-9B", "status": "failed", "error": str(e)}
            )
    ...
    def search_gpu_benchmarks(self) -> Dict:
        ...
        results = {"source": "Various benchmarks", "gpus": [], "queries": []}
        ...
        try:
            ...
            if response.status_code == 200:
                results["queries"].append(
                    {"query": "Wikipedia CUDA", "status": "success", "url": url}
                )
        except Exception as e:
            results["queries"].append(
                {"query": "Wikipedia CUDA", "status": "failed", "error": str(e)}
            )
...
def main():
    ...
    # 保存结果到 JSON 文件
    output_file = (
        "/Users/hubertshelley/Documents/silent/tiangong/src-tauri/query_results.json"
    )
