...
// ---- 临时桥接实现（G-3 时迁移到 plugin） ----

/// 通过 BrowserCommand channel 实现 PageFetcher trait
pub struct ChannelPageFetcher {
    cmd_tx: tokio::sync::mpsc::Sender<tiangong_types::BrowserCommand>,
}

impl ChannelPageFetcher {
    pub fn new(cmd_tx: tokio::sync::mpsc::Sender<tiangong_types::BrowserCommand>) -> Self {
        Self { cmd_tx }
    }
}

impl tiangong_core::browser_trait::PageFetcher for ChannelPageFetcher {
    fn fetch_page(
        &self,
        url: &str,
        max_chars: usize,
...
impl BrowserToolOverride {
    pub fn new(fetcher: std::sync::Arc<dyn tiangong_core::browser_trait::PageFetcher>) -> Self {
        Self { fetcher }
    }
}
