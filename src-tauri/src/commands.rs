...
fn validate_editable_message(
    ...
    if message.phase == tiangong_core::session::MessagePhase::ModelOnly {
        return Err(anyhow::anyhow!("该消息为宿主注入的模型专用消息，无法编辑"));
    }
...
pub async fn prepare_startup_resources(
    ...
    match result {
        ...
        Err(error) => {
            ...
            // 失败原因经返回值同步交给前端提示：不放事件——主界面挂载
            // 晚于此处的 emit，事件在首启场景必然丢失。
