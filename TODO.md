...
- [x] LLM 层识别并剔除参数不完整的工具调用，不向 Agent 返回伪造的参数对象。
- [x] LLM 层内部只请求一次参数修正，并合并两次请求的用量与失败诊断。
- [x] Agent 删除流式参数错误识别、修正计数和专用收尾逻辑，只保留诊断展示、保存与审计。
- [x] 完成 LLM、Core 和 Coding 插件构建验证，确认残缺调用不会执行且不会触发第三次修正。

### 本次完成标准

- Agent 代码不再检查 `__parse_error`，也不再维护流式参数修正状态。
- 第一次参数失败、第二次成功时只执行修正后的完整调用；连续两次失败时不执行工具并直接结束。
- 两次失败均可在会话、界面和审计记录中看到，LLM 请求总数严格为两次。
