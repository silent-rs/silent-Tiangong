...
- [x] 支持文件拖动到 GUI 用户输入框添加为附件并预览。
- [x] Desktop 发送附件前统一转为 base64 data URL，超 50MB 停止请求。
- [x] 包含附件的用户消息优先使用多模态模型解析。
- [x] 未配置多模态模型时关闭文件上传能力。
- [x] 用户输入图片附件由 Core 归档到本地。
- [x] 图片生成结果优先归档到本地并引用本地图片。
...
- [x] 实现 `text` 模式 URL 校验、权限策略、超时、重定向、响应体大小限制。
- [x] 实现 `download` 模式路径校验、覆盖控制、流式写入、大小限制和摘要计算。

### Core 多轮对话上下文整理 ✅

- [x] Core 层将 recall_memory 结果作为消息上下文注入。
...
- [x] 命令执行 cwd 限制在写入允许范围内。

### RFC-0007 Skill 文件系统注册表 ✅

- [x] 定义 Skill 文件系统注册表数据结构。
- [x] 支持 `skill.toml.available` 字段。
- [x] 实现轻量扫描与按需加载。
- [x] 改造 SkillService 主链路。
- [x] 移除 `skills.installed[]` 持久化写入。
- [x] 实现旧布局迁移器。
- [x] 刷新与 GC 命令。
- [x] Skill 激活期 MCP 缺失处理。
- [x] UI 管理面板懒加载。
- [x] `skill doctor` 诊断工具。
- [x] 删除旧注册机制。
- [x] 文档与示例更新。
