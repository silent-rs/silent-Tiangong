...
- [ ] 插件服务平台短期暂停；当前只维护官方签名、OSS 静态目录与客户端安装升级链路。
...
## 当前任务：GitHub Actions 官方插件 OSS 发布闭环

- [x] 新增独立插件发布工作流，每次输入一个插件 ID，由仓库构建配置核对目录完整性，不在 Actions 下拉选项中重复维护插件列表；随后在 macOS Apple Silicon、Linux x86_64 和 Windows x86_64 分别构建签名。
- [x] 每个平台构建后上传短期 Actions Artifact，签名私钥只写入临时文件并在构建后删除。
- [x] 增加单插件多平台制品合并命令，校验插件 ID、版本、说明、清单、WASM、sidecar 和签名平台的一致性。
- [x] 每次发布只上传所选插件版本和独立入口；发布阶段使用全局串行锁读取线上目录、替换该插件条目并更新总目录，保留其他插件。
- [x] 发布前要求每个插件同时具备三个目标平台，拒绝缺少平台、重复平台或共享制品冲突。
- [x] OSS 发布先上传版本制品和目录片段，最后更新 `plugins-index/catalog.json`，并对线上目录执行 SHA-256 回读验证。
- [x] 应用安装包发布流程不再重复构建插件；插件 OSS 发布可由版本标签或手动触发独立执行。
- [x] 同步 `PLAN.md` 与插件开发指南，明确短期只维护 OSS 静态发布链路，不建设插件服务平台。

### 当前任务完成标准

- 三个平台任一构建失败时不会更新所选插件或线上总目录。
- 一次发布只构建、签名和上传一个插件，不受其他插件构建状态影响。
- 合并目录只包含所选插件三个平台内容一致、签名制品完整的一个版本。
- 更新总目录时保留其他插件，多个插件同时完成构建后也必须串行发布，不能互相覆盖。
- OSS 客户端可先下载完整制品，再通过最后发布的目录发现版本，不会看到半发布状态。
- 正式签名私钥不进入仓库、构建产物或长期工作目录。
