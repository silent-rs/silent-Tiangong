...
- 修复 Agent 主循环中 GLM 5.1 等长思考模型可能因流式生成超时或输出上限提前停止的问题。
- 主 Agent 流式生成不设置整体超时，主模型生成默认传入 `1024 * 1024` 的 `max_tokens` 高上限。
- LLM 输出后按输入与输出 token 总量触发上下文压缩，避免长输出影响后续任务继续执行。
