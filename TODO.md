...
- [x] 梳理 GUI / Server / Gateway 当前入口差异，确认统一事件模型的目标边界
...
- [x] 将 OpenAI 兼容 transport 拆分保留为后续子任务，不阻塞当前主线功能

### D. 安全、审计与观测补齐

- [x] 盘点路径级、网络目标级和外部作用域控制点的现状缺口
- [x] 补齐高风险动作审计字段，覆盖会话、任务、代理、决策和结果摘要
- [x] 对齐请求级 / 任务级 / 会话级成本统计与用户可见状态

## Phase 17：Server 信任语义收敛 — **已完成**

- [x] Server 模式启动和配置同步时强制使用 `full_trust`
- [x] 删除 Server 端审批 API、审批事件和远程审批恢复逻辑
- [x] 将远程角色从 `controller / approver / observer` 收敛为 `controller / observer`
- [x] 复核 Server 剩余接口与事件推送，确保只保留信任模式下需要的远程能力

## Phase 18：远程能力尾项收口 — **待启动**

- [ ] 为 Server 补充请求级 / 任务级 / 会话级成本查询接口与可见范围控制
- [ ] 继续收敛 GUI 与 Server 的入口差异，评估是否统一到同一 ingress 抽象
- [ ] 清理远程能力相关历史文档和阶段描述，避免继续保留旧审批语义
