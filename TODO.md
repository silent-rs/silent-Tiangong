...
- [ ] stdio 普通请求并发分发并设置上限；停止请求绕过上限，主循环不等待长任务。
- [ ] 按请求编号取消，建立会话、工具调用和 sidecar 请求关联；只取消目标任务。
- [ ] terminal、command 及其他子进程插件收到取消后清理目标进程树并释放占用。
- [ ] 活跃任务中止立即投递；仅发送准备阶段等待发送完成后重试。
- [ ] 思考强度只更新当前会话，不刷新其他 Core/插件，不跨等待持有会话锁。
- [ ] 长 sidecar 调用移出界面命令处理线程。
- [ ] 前端停止后立即显示“正在取消”，后端超时显示仍在运行与重试提示，真实结束前不显示空闲。
- [ ] 同步 Rust sidecar 公共库、Node SDK 与官方 sidecar 制品版本；旧协议明确要求升级且不降级。
- [ ] 验证并发中止、目标进程退出、跨会话隔离、切换思考强度后停止、连续取消无残留。
