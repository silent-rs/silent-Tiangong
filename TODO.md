...
- [x] 已经发出的模型请求不取消、不重启，当前任务结束时保存最新思考强度。
- [x] 后续发送和重新打开对话继续使用会话保存值，不被应用默认值覆盖。
- [x] 同步核对会话 TrustMode，避免同一发送路径覆盖会话级设置。
- [x] 通过相关 Rust 验证、严格检查和前端正式构建。

## 完成标准

- 运行中从任意强度切换后，当前请求保持不变，工具执行后的后续模型请求使用新强度。
- 当前任务完成、取消或失败后，重新加载会话仍显示并使用最后选择的强度。
- 新对话使用创建时选择的强度，已有对话始终使用各自保存的思考强度与 TrustMode。
...
- [x] 移除工具调用默认附加的 120 秒上限，使用模型供应商配置的超时时间。
- [x] 保持有效 `API_FUNCTION_TIMEOUT_MS` 对工具调用超时的显式覆盖能力。
- [x] 确认流式工具调用和非流式回退使用一致的超时时间选择规则。
- [x] 通过 `tiangong-llm` 针对性测试、编译检查和格式检查。

## 完成标准

- 模型供应商配置为 300 秒时，工具调用请求实际使用 300 秒，不再被缩短为 120 秒。
- 未设置工具调用专用覆盖时，所有主模型请求使用模型供应商配置。
- 显式设置有效工具调用专用覆盖时继续采用覆盖值。
...
## 当前回归修复：终端命令假执行与关闭后的悬挂

- [x] 历史终端输出只按静态文本恢复，不让其中的终端查询再次向当前 PTY 回写响应。
- [x] 非交互命令必须收到启动标记才进入正常执行等待；启动失败时短时间内结束，不沿用命令总超时。
- [x] 用户关闭终端 Tab 或销毁终端会话时，立即结束绑定的 PTY 及普通、交互命令等待。
- [x] 工具结果明确区分命令超时、命令未启动和终端已关闭，不把终端故障当作执行成功。
- [x] 使用真实长命令验证关闭后两秒内返回，并通过终端插件检查和前端正式构建。
- [ ] 历史终端输出只按静态文本恢复，不让其中的终端查询再次向当前 PTY 回写响应。
- [ ] 非交互命令必须收到启动标记才进入正常执行等待；启动失败时短时间内结束，不沿用命令总超时。
- [ ] 用户关闭终端 Tab 或销毁终端会话时，立即结束绑定的 PTY 及普通、交互命令等待。
- [ ] 工具结果明确区分命令超时、命令未启动和终端已关闭，不把终端故障当作执行成功。
- [ ] 通过终端插件检查、主程序检查和前端正式构建。

### 本次完成标准

- 历史输出中的颜色和光标查询不会污染下一条命令，短命令能实际发送并及时返回。
- 没有收到启动标记的命令不会显示为长时间执行，也不会等满 Agent 设置的总超时。
- 关闭正在执行命令的终端后，Agent 在两秒内收到失败结果并可继续后续处理。
- 关闭正在执行命令的终端后，Agent 及时收到失败结果并可继续后续处理。
- 已关闭终端中的旧命令不会在新终端中恢复或继续执行。
- 关闭空闲终端和关闭整个终端会话仍能正常回收 PTY。

## 当前回归修复：Agent 命令前终端清理

- [x] Agent 普通和交互命令发送前使用 `Ctrl+C` 取消完整输入，不再依赖只能清理当前行的 `Ctrl+U`。
- [x] 清理后通过内部标记确认 Shell 已恢复，确认前不发送真实命令。
- [x] Shell 无法恢复时废弃旧 PTY 并重建，重建后仍未就绪则明确返回失败。
- [x] 内部就绪标记不显示在终端或写入持久日志。
- [x] 使用真实 PTY 验证未闭合多行输入、不响应 `Ctrl+C` 的前台任务和正常连续执行。

### 本次完成标准

- 未闭合的多行输入不会与下一条 Agent 命令拼接。
- Agent 命令只在 Shell 明确就绪后发送，旧终端异常时能自动换用新 PTY。
- 终端中不出现内部就绪命令或标记，正常命令执行和终端复用不受影响。

## 当前功能：终端 Tab 繁忙标记

- [x] 将终端实时阶段同步到工作区 Tab 状态。
- [x] 为繁忙终端 Tab 显示固定尺寸的黄色状态点，空闲和浏览器 Tab 不显示。
- [x] 终端恢复空闲时主动通知前端刷新，避免状态点滞留。
- [x] 通过前端正式构建、终端插件检查和真实界面验证。

### 本次完成标准

- 普通命令和交互任务进入繁忙阶段后，对应终端 Tab 显示黄色状态点。
- 终端恢复空闲后，黄色状态点无需刷新页面即可消失。
- 状态点不挤压 Tab 标题、图标和关闭按钮，明暗主题下均清晰可见。
...
## 当前任务：插件 OSS 目录发布可靠性修复

- [x] 纯 WASM 插件目录使用空对象表示无 sidecar，不再生成客户端无法读取的 `null`。
- [x] 合并后的完整目录在上传前按客户端目录结构、版本、地址和校验值规则验证。
- [x] OSS 当前目录下载失败时停止发布，不再以空目录继续覆盖线上数据。
- [x] 正式目录替换前保存上一份目录，上传后回读验证失败时立即恢复。
- [x] 文档明确多个插件首次发布需分批串行触发，后续独立发布继续一次处理一个插件。
- [x] 受控重发 Prompt 插件并回读 OSS，确认现有全部插件目录可正常解析。

### 当前任务完成标准

- 任一目录条目结构无效时发布在写入 OSS 前失败，不能影响其他插件。
- 网络异常不会把已有总目录降为只包含本次插件的空白目录。
- 发布后线上目录与本地结果一致且可被客户端完整读取，失败时恢复上一份目录。
- 当前 OSS 总目录保留已有全部插件，Prompt 条目不再包含无效空值。

## 当前任务：插件管理条目与版本升级体验

- [x] 重构已安装插件条目的信息层级，突出插件名称、运行状态和启停开关。
- [x] 显示当前版本；OSS 存在更高版本时显示可升级版本和明确的升级按钮。
- [x] 移除面向内部实现的“第 N 代”显示。
- [x] 优化 sidecar 状态展示，区分不需要 sidecar、运行中、待启动和异常。
- [x] 保留热加载、回滚和卸载能力，并让不可用操作原因清晰可见。
- [x] 通过前端正式构建并核对典型状态下的页面展示。

### 当前任务完成标准

- 用户无需理解内部代数即可判断插件是否正常、当前版本和是否可升级。
- 可升级插件可从已安装列表直接完成升级，升级后版本和状态立即刷新。
- sidecar 状态使用易懂文案和状态颜色，不将未运行一律误报为异常。

## 当前任务：插件加载时机与管理页刷新隔离

- [x] 插件只在应用启动阶段完成预加载；读取插件状态和进入设置页不得隐式触发加载。
- [x] Core 创建和会话切换只使用启动时已接受的插件快照，不重复扫描、编译或热加载插件。
- [x] 热加载只由用户主动发起；制品未变化时直接返回当前状态，不创建新实例或增加代数。
- [x] 安装、升级、回滚和启停继续作为明确的用户操作更新插件快照。
- [x] 插件管理页刷新期间显示全页遮罩，禁止切换页签、启停或执行其他插件操作。
- [x] 验证启动预加载、状态查询、无变化热加载、会话插件构造和前端刷新交互。

### 当前任务完成标准

- 打开设置或插件配置页不会出现插件重复加载日志，也不会启动新的 sidecar。
